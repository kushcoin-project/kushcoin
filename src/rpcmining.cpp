...
Value getmininginfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("stakeinterest", GetProofOfStakeReward(0, GetLastBlockIndex(pindexBest, true)->nHeight, GetLastBlockIndex(pindexBest, true)->nTime, true)));
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "KushCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "KushCoin is downloading blocks...");
