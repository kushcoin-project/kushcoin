...
unsigned int nStakeMinAge = 10 * nOneHour; // as zero time weight
unsigned int nStakeMaxAge = 30 * nOneHour; // as full weight
unsigned int nStakeTargetSpacing = (4 * 60) + 20; // 4-minute 20 second stakes spacing
//int64_t nTargetSpacing = 1.2 * 60;
int64_t nTargetSpacingPOW = 1.2 * 60; // 72 second POW spacing
...
int64_t GetProofOfWorkReward(unsigned int nBits, int64_t nFees, int nHeight)
{
	...
	if (nHeight < 1200)
		...
		nSubsidy = 200 * COIN;
	else if (nHeight < 15600)
		nSubsidy = 180 * COIN;
	else if (nHeight < 16802)
		nSubsidy = 160 * COIN;
...
static const int64_t nTargetTimespan = 1.2 * 15 * 60;
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
	...
	int64_t nTargetSpacing = fProofOfStake? nStakeTargetSpacing : nTargetSpacingPOW;
	...
	int64_t nTargetTimespan0 = nTargetTimespan;
	
	if (fProofOfStake){
		nTargetTimespan0 = 4 * 15 * 60; }
	
    int64_t nInterval = nTargetTimespan0 / nTargetSpacing;
    ...
    if (!IsCoinBase())
    {
        ...
        if (IsCoinStake())
        {
            if (nTime >  Checkpoints::GetLastCheckpointTime())
            {
                ...
                int64_t nCalculatedReward = GetProofOfStakeReward(nCoinAge, pindexBlock->nHeight, nTime) - GetMinFee(1, false, GMF_BLOCK, nTxSize) + CENT;
...
void ThreadScriptCheck(void*) {
    ...
    RenameThread("KushCoin-scriptch");
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in KushCoin
...
bool CBlock::SetBestChain(CTxDB& txdb, CBlockIndex* pindexNew)
{
    ...
    printf("SetBestChain: new best=%s  height=%d  trust=%s  blocktrust=%" PRId64 "  date=%s time=%d\n",
      hashBestChain.ToString().c_str(), nBestHeight,
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (fProofOfStake)
    {
        ...
        // KushCoin: check proof-of-stake block signature
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    if (!Checkpoints::CheckBanned(hash)) {
        ...
        return error("ProcessBlock() : block %s is rejected by hard-coded banlist", hash.GetHex().c_str());
    }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "KushCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "KUSH Genesis";
        CTransaction txNew;
        ...
        block.nNonce   = 5264225; // !fTestNet ? 1575379 : 46534;
       ...
       if (true && (block.GetHash() != hashGenesisBlock)) {
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x96e11ebbed568155907c007a89f16c977a8d288f7f0412396ddf4a61154b1d2f"));
