...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #KushCoinTEST2\r");
            Send(hSocket, "WHO #KushCoinTEST2\r");
        } else {
            // randomly join #KushCoin00-#KushCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #niochsuk%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #niochsuk%02d\r", channel_number).c_str());
        }
